...
<div class="row">
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header bg-warning text-light">officer Targets</div>
            <div class="card-body">
                <div class="row">
                    <div class="col">
                        <a href="{{ url('upload-officer-targets') }}" class="btn btn-warning btn-sm text-light" title="Add officer Targets">
                            <i class="fa fa-plus" aria-hidden="true"></i> Add New
                        </a>
                    </div>
                    <div class="col">
                        <a href="{{ url('download-officer-targets-template') }}" class="btn btn-warning btn-block text-light" title="Add officer Targets">
                ...
                <form method="GET" action="{{ url('/officer-targets-uploader') }}" accept-charset="UTF-8"
                        ...
                        <tbody>
                            @foreach ($targets as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->officer->names }}</td>
                                    <td>{{ number_format($item->target_amount) }}</td>
                                    <td>{{ number_format($item->target_numbers) }}</td>
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        if (xhr.responseJSON && xhr.responseJSON.errors && xhr.responseJSON
                            .errors.officer_targets_file) {
                            errorMessage = xhr.responseJSON.errors.officer_targets_file[0];
                        } else {
